...
def run_export(opt: SimpleNamespace) -> None:
    ...
    Accepts an object (e.g. argparse.Namespace or SimpleNamespace) with attributes:
    weights, img_size, batch_size, onnx_only, dynamic, dynamic_batch, grid,
    end2end, max_wh, topk_all, iou_thres, conf_thres, device, simplify,
    include_nms, fp16, int8.
    '''
    ...
    if not opt.onnx_only:
        ...
        try:
            ...
            ts = None  # ensure ts is defined for later use
        ...
        try:
            ...
            if ts is None:
                raise RuntimeError('TorchScript model not available for CoreML export.')
        ...
        if not opt.onnx_only:
            # Lazy import: only try to pull in mobile optimizer if we actually need it
            global optimize_for_mobile
            if optimize_for_mobile is None:
                try:
                    from torch.utils.mobile_optimizer import optimize_for_mobile as _ofm
                    optimize_for_mobile = _ofm
                except Exception as e:
                    print(
                        'Warning: torch.utils.mobile_optimizer could not be imported; '
                        'TorchScript-Lite export will be skipped.\n  Details:', e
                    )
                    optimize_for_mobile = None

            if optimize_for_mobile is not None:
                try:
                ...
                print('Skipping TorchScript-Lite export (mobile optimizer unavailable).')
        else:
            print('Skipping TorchScript-Lite export (onnx_only=True).')
...
def export_model(
    ...
    '''
    High-level API for use from notebooks / other Python code.
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--onnx-only', action='store_true', help='only onnx export')
